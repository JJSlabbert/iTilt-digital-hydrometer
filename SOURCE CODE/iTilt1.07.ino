...
//ESP32 POWER PIN
  ...
  //#include "esp_err.h"
#endif
char mpu_orientation[2]="1"; // If The iTilt is in a wort, if Vcc is on Top, this should be 1.
...
void startDeepSleep()
{
  ...
  gpio_hold_en(GPIO_NUM_2);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep  
  gpio_hold_en(GPIO_NUM_33);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en(GPIO_NUM_0);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  //GPIO 1 and 3 ist TXD and RXD

  gpio_hold_en(GPIO_NUM_4);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en(GPIO_NUM_5);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en(GPIO_NUM_12);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en(GPIO_NUM_13);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_14);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_15);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_16);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep    
  gpio_hold_en (GPIO_NUM_17);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_18);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_19);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_21);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_22);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep 
  gpio_hold_en (GPIO_NUM_23);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep 
  gpio_hold_en (GPIO_NUM_25);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep 
  gpio_hold_en (GPIO_NUM_26);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep 
  gpio_hold_en (GPIO_NUM_27);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep 
  gpio_hold_en (GPIO_NUM_32);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep
  gpio_hold_en (GPIO_NUM_33);  //https://github.com/espressif/arduino-esp32/issues/2712 To keep GPIO power pin LOW during Deep Sleep                  
...
{ 
  ...
  for (int i=0;i<1000;i++)
  {
    ...
    if (i==999)
    {
      Serial.println("You are in calcRoll(). The MPU6050 could not provide numerical readings for 1000 times.");
    }
  }
...
void handlePolynomialCalibrationStart()
{
  ...
  htmlText+="<pIt is recomended to use a sample size of 7 and the and water volumes and sugar weights provided in the next step.</p>";  
...
void handleReadings() {
  ...
  htmlText+="<p>Roll: "+String(roll)+" Degrees. This value should be close to 0. </p>";
...
{ 
  ...
  Serial.println("Topic 0: "+topic0);
  Serial.println("iTilt ID Number: "+String(itiltnum));
  Serial.println("Topic 1: "+topic1);
  Serial.println("Battery Capacity: "+String(batcap,0));
  Serial.println("Topic 2:"+topic2);
  Serial.println("Topic 2: "+String(tempgyro,2));
  ...
  //mqttClient.print(String(fcoefficientx3)+"," +String(coefficientx2)+","+ String(coefficientx1)+","+String(constantterm)); 
  //mqttClient.endMessage();
  
  mqttClient.beginMessage(topic0);
  mqttClient.print(itiltnum); 
  mqttClient.endMessage();
  mqttClient.beginMessage(topic1);
  mqttClient.print(batcap); 
  mqttClient.endMessage();       
  mqttClient.beginMessage(topic2);
  mqttClient.print("temp,c="+String(tempgyro-0.53,2)); 
  mqttClient.endMessage();
  mqttClient.beginMessage(topic3);
  mqttClient.print(1.06); 
  mqttClient.endMessage();
  ...
  mqttClient.endMessage();
  delay(1000);  //otherwise some of the cayenne channels is not publish
  if (signalstrength<90)//otherwise some of the cayenne channels is not publish
  {
    delay(500);
  }
  if (signalstrength<80)//otherwise some of the cayenne channels is not publish
  {
    delay(600);
  }
  if (signalstrength<60)//otherwise some of the cayenne channels is not publish
  {
    delay(1000);
  }
  if (signalstrength<40)//otherwise some of the cayenne channels is not publish
  {
    delay(1500);
  }  
