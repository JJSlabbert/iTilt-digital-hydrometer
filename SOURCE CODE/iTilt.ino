...
void setup() {
  ...
  if (SPIFFS.begin()) {
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        Serial.println("ARDUINOJSON VERSION 6");
  ...
  float tilt=calcTilt(); //
  batvolt = analogRead(A0) / atof(batconvfact);
...
CAYENNE_OUT_DEFAULT()
{
  ...
  float fcoefficientx1=atof(coefficientx1);
  float fconstantterm=atof(constantterm);     
  String polynome=String(fcoefficientx3,8)+", "+String(fcoefficientx2,6)   +", "+String(fcoefficientx1,5)+", "+String(fconstantterm,5);
  ...
  Cayenne.virtualWrite(2, polynome.c_str());

  Cayenne.virtualWrite(3, "Firmware Version: 1.02");
  ...
  {
    Serial.println("The iTilt is vertical, Did you use a magnet? Wifi Manager configuration portal will start");
    ESP.restart();
  }
}
